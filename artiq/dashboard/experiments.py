...
class _ExperimentDock(QtWidgets.QMdiSubWindow):
    ...
    def _create_due_date_widgets(self):
        ...
        def update_datetime_en(checked):
            if checked:
                ...
                # There is some inconsistency here. We should read the default value,
                # but there is only default value of the 'Due date', which is None.
                # That means that the checkbox should be un-checked.
                # We could check here if the default value is None or not, bot even
                # if it was not None, it wouldn't necessary mean that the checkboax
                # should be checked. We allow here hardcoded default value 'un-checked'.
    ...
    def _create_pipeline_widgets(self):
        ...
        def update_pipeline_name(text):
            ...
            if text != self.pipeline_default_value:
                self.pipeline_widget_name.setStyleSheet("background-color: grey;")
                logger.warning("Changing pipeline name to non-default value")
            else:
                self.pipeline_widget_name.setStyleSheet("background-color: transparent;")
    ...
    def _create_priority_widgets(self):
        ...
        def update_priority(value):
            ...
            if value != self.priority_default_value:
                self.priority_widget_name.setStyleSheet("background-color: grey;")
                logger.warning("Changing priority to non-default value")
            else:
                self.priority_widget_name.setStyleSheet("background-color: transparent;")
        # Make sure that non-default values are notified
    ...
    def _create_flush_widgets(self):
        ...
        composite_widget.setToolTip("Flush the pipeline (of current- and higher-priority "
    ...
    def _create_devarg_override_widgets(self):
        ...
        self._devarg_override_icon_action = self.devarg_override.lineEdit().addAction(
                icon_svg, QtWidgets.QLineEdit.LeadingPosition)
        ...
        if "repo_rev" in self.options:
            ...
            def update_repo_rev(text):
                ...
                if self.options["repo_rev"] != scheduling_options["repo_rev"]:
                    self.repo_rev_label.setStyleSheet("background-color: grey;")
                    logger.warning("Changing 'Rev / ref' to non-default value")
                else:
                    self.repo_rev_label.setStyleSheet("background-color: transparent;")
