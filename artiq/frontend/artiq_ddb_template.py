...
def get_cpu_target(description):
    ...
    if description["target"] == "kasli":
        if description["hw_rev"] in ("v1.0", "v1.1"):
            ...
            return "rv32g"
    elif description["target"] == "kasli_diot":
        return "rv32g"
...
def process_header(output, description):
    ...
    """).format()
    if description["target"] in ["kasli", "kasli_soc"]:
        i2c_switches += textwrap.dedent("""
            "i2c_switch1": {{
                "type": "local",
                "module": "artiq.coredevice.i2c",
    ...
    # FIXME: proper path to SERVMOD and IO expander
    if description["target"] in ["kasli_diot"]:
        io_expander = textwrap.dedent("""
        "servmod_expander": {{
            "type": "local",
            "module": "artiq.coredevice.diot_i2c",
            "class": "PCA9539",
            "arguments": {{"address": 0xec}}
        }},
        """).format()
        servmod = textwrap.dedent("""
        "servmod_switch": {{
            "type": "local",
            "module": "artiq.coredevice.diot_i2c",
            "class": "ServmodSwitch",
            "arguments": {{
               "sw_device": "i2c_switch0",
                "servmod_device": "servmod_expander",
            }}
        }},
        """).format()
    ...
    print(textwrap.dedent("""
        ...
        """).format(
            ...
            i2c_switches=textwrap.indent(i2c_switches, " "*4),
            io_expander=textwrap.indent(io_expander, " "*4) if description["target"] in ["kasli_diot"] else "",
            servmod=textwrap.indent(servmod, " "*4) if description["target"] in ["kasli_diot"] else ""),
        file=output)
...
class PeripheralManager:
    ...
    def process_ohwr_diot_loopback(self, rtio_offset, peripheral):
        loopback_name = self.get_name("ohwr_diot_loopback")

        self.gen("""
                device_db["{name}_lvds0_counter"] = {{
                    "type": "local",
                    "module": "artiq.coredevice.edge_counter",
                    "class": "EdgeCounter",
                    "arguments": {{"channel": 0x{channel:06x}}},
        ...
        for i in range(7):
            self.gen("""
                device_db["{name}_lvds{lvds}_out"] = {{
                    "type": "local",
                    "module": "artiq.coredevice.ttl",
                    "class": "TTLOut",
                    "arguments": {{"channel": 0x{channel:06x}}}
                ...
                name=loopback_name,
                lvds=1+i*2+0,
                channel=rtio_offset+1+i*2)
            self.gen("""
                device_db["{name}_lvds{lvds}_in"] = {{
                    "type": "local",
                    "module": "artiq.coredevice.ttl",
                    "class": "TTLInOut",
                    "arguments": {{"channel": 0x{channel:06x}}}
