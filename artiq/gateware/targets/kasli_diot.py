...
from misoc.targets.kasli_diot import (
...
from artiq.gateware import rtio, diot, diot_7series
...
class MCXClkinForward(Module):
    def __init__(self, platform):
        sma_clkin = platform.request("mcx_clkin")
        sma_clkin_se = Signal()
...
class StandaloneBase(MiniSoC, AMPSoC):
    ...
    def __init__(self, gateware_identifier_str=None, with_wrpll=False, hw_rev="v2.0", **kwargs):
        ...
        if with_wrpll:
            ...
            self.submodules += MCXClkinForward(self.platform)
            self.config["HAS_SI5324"] = None
            self.config["SI5324_SOFT_RESET"] = None

        i2c_pads = []
        for i in range(4):
            i2c = self.platform.request("i2c", i)
            i2c_pads += [i2c.scl, i2c.sda]
        self.submodules.i2c = gpio.GPIOTristate(i2c_pads)
        self.csr_devices.append("i2c")
        self.config["I2C_BUS_COUNT"] = 4
...
#     def __init__(self, rtio_clk_freq=125e6, with_wrpll=False, 
#                  gateware_identifier_str=None, hw_rev="v1.0", **kwargs):
#         cpu_bus_width = 64
#         MiniSoC.__init__(self,
#                          cpu_type="vexriscv",
#                          hw_rev=hw_rev,
...
#         for i in range(4):
#             i2c = self.platform.request("i2c", i)
#             i2c_pads.append([i2c.scl, i2c.sda])
#         self.submodules.i2c = gpio.GPIOTristate(i2c_pads)
#         self.csr_devices.append("i2c")
#         self.config["I2C_BUS_COUNT"] = 4
...
class GenericStandalone(StandaloneBase):
    def __init__(self, description, hw_rev=None,**kwargs):
        ...
        self.rtio_channels = []
        diot_7series.add_peripherals(self, description["peripherals"])
        for i in range(3):
...
def main():
    ...
    parser.set_defaults(output_dir="artiq_kasli_diot")
    ...
    if description["target"] != "kasli_diot":
